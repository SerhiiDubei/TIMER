...
CREATE INDEX IF NOT EXISTS idx_players_eliminated ON players(eliminated_at) WHERE eliminated_at IS NULL;
...
CREATE INDEX IF NOT EXISTS idx_codes_used ON codes(used_at) WHERE used_at IS NULL;
...
CREATE INDEX IF NOT EXISTS idx_events_target_player ON events(target_player_id);
...
DROP POLICY IF EXISTS "Service role can modify rooms" ON rooms;
DROP POLICY IF EXISTS "Anyone can read players" ON players;
DROP POLICY IF EXISTS "Service role can modify players" ON players;
DROP POLICY IF EXISTS "Service role can access codes" ON codes;
DROP POLICY IF EXISTS "Anyone can read events" ON events;
DROP POLICY IF EXISTS "Service role can modify events" ON events;
